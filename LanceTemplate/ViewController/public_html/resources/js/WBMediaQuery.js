...
(function (wbMQ) {
   ...
   */
  wbMQ.setupMediaQuery = function(mediaQueryStringArr,cusLocArr, mqRootId, mqLayoutProp, initialMatchValue, matchBtnId)
  {
    if(AdfPage.PAGE.isSynchronizedWithServer())
    {
      ...
      if (window.matchMedia) // supported only on modern browsers
      {
        ...
        var currentActualLayout = null;
        var currentActualQuery = null;
        for(var t=0;t<mediaQueryStringArr.length;t++){
            var mediaQueryString = mediaQueryStringArr[t];
            compMQs[mediaQueryString] = {'defLoc':cusLocArr[t],'mqRootId': mqRootId, 'idPrefix': idPrefix, 'matchBtnId': matchBtnId};
            ...
            if(!compMQs[mediaQueryString].mqList)
            {
              ...
              if(mqList.matches){
                  //获取当前实际屏幕类型
                  currentActualLayout = mqList.compMQ.defLoc; 
                  currentActualQuery  = mediaQueryString;
              }
            }
        } 
        ...
        var func = function ()
        {
          ...
          // initial check
          if ((currentLayout == initialMatchValue || !currentLayout) && currentActualLayout != currentLayout)
          {
            changeLayout(false, contentContainer, compMQs[currentActualQuery]);
          }
        }
    ...
    {
      window.setTimeout(function(){wbMQ.setupMediaQuery(mediaQueryStringArr,cusLocArr, mqRootId, mqLayoutProp, initialMatchValue, matchBtnId)}, 100);
    }
  }
  ...
  function mediaQueryListener(mediaQueryList)
  {
    ...
    if (contentContainer && mediaQueryList.matches)
    {
  ...
  {
    var button = AdfPage.PAGE.findComponent(compMQ.idPrefix + compMQ.matchBtnId);
    button.setText(compMQ.defLoc);
//    button.setProperty("defLoc",compMQ.defLoc);
